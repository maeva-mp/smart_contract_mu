...
# ============================================================================
# LEGAL KNOWLEDGE BASE - MAURITIUS LAW
# ============================================================================

# Contract Risk Keywords
RISK_KEYWORDS = {
    "critical": [
        "unlimited liability", "jointly and severally liable", "personal guarantee",
        "indemnify and hold harmless", "waive all rights", "waiver of rights",
        "terminate without cause", "unilateral termination", "terminate immediately without notice",
        "exclusive remedy", "sole remedy", "liquidated damages", "penalty clause",
        "penalty of", "unconditionally and irrevocably", "without first pursuing",
        "unlimited and joint"
    ],
    "high": [
        "indemnify", "indemnification", "hold harmless", "limitation of liability",
        "liable for damages", "breach of contract", "event of default", "material breach",
        "warranty disclaimer", "as is", "non-compete", "non-solicitation",
        "confidential information", "proprietary information", "intellectual property rights",
        "binding arbitration", "waive any right"
    ],
    "medium": [
        "termination notice", "notice period", "payment terms", "late payment",
        "force majeure", "renewal", "amendment", "governing law", "dispute resolution",
        "confidentiality", "delivery deadline", "milestone"
    ]
}

# Mauritius Compliance Patterns
...
# Civil Code Articles (Enhanced with explanations)
CIVIL_CODE_ARTICLES = {
    "1108": {
...
# Legal Recommendations by Article
...
EXCLUDED_ORG_WORDS = {
    ...
    "agreement", "contract", "court", "government", "human resources",
    "managing director", "chief executive officer", "director", "manager",
...
EXCLUDED_PERSON_WORDS = {
    ...
    "director", "manager", "employer", "employee", "tenant", "landlord",
    "date", "witness", "notary", "notary public", "the guarantor"
}
...
def load_ground_truth(path: str = GROUND_TRUTH_FILE) -> Dict:
    """Load ground truth query-document pairs for evaluation"""
    try:
...
def chunk_text(text: str, approx_tokens: int = CHUNK_TOKENS, 
...
def load_spacy_model():
    """Load spaCy NLP model with automatic download if needed"""
    try:
        ...
        try:
            ...
            with st.spinner(f"📥 Downloading {SPACY_MODEL_NAME} (one-time, ~12MB)..."):
                try:
        ...
        # Add entity ruler with legal patterns
...
@st.cache_resource(show_spinner=False)
def load_models():
    """Load all ML models with progress indicators"""
...
def extract_entities(text: str, nlp) -> Dict[str, List[str]]:
    ...
    try:
        ...
        for ent in doc.ents:
            ...
            # Basic filters
            if len(ent_text) < 3 or ent_text.count('(') != ent_text.count(')'):
                continue
            if ent_text.isupper() and len(ent_text) > 4:
                continue
            if any(ent_lower.startswith(p) for p in ['section ', 'article ', 'clause ']):
                continue
            ...
            if ent.label_ == "DATE":
                ...
                if (ent_lower not in MAURITIUS_LOCATIONS and 
                    ent_lower not in EXCLUDED_ORG_WORDS and
                    "registered" not in ent_lower):
                    
                    is_company = any(x in ent_lower for x in ["ltd", "co.", "inc", "llc", "limited", "corp"])
                    is_proper_name = ent_text[0].isupper() and len(ent_text) > 3
                    
                    if is_company or is_proper_name:
                        entities["orgs"].append(ent_text)
            
            elif ent.label_ == "PERSON":
                if (ent_lower not in MAURITIUS_LOCATIONS and
                    ent_lower not in EXCLUDED_PERSON_WORDS and
                    ent_text[0].isupper()):
...
def detect_risk_level(text: str) -> Tuple[str, List[Tuple[str, str]], float]:
    """Detect risk level and matched keywords"""
